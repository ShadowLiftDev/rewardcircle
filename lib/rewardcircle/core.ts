...
import {
  ...
  memberRef,
  membersCollection,
  rewardCircleConfigRef,
  rewardCircleMemberStateProjectionRef,
  rewardCircleProgramRef,
  rewardCircleRewardRef,
  rewardCircleRewardsCollection,
  rewardCircleStateRef,
} from "./paths";
...
export async function findRewardCircleMemberByPhone(
  ...
  const stateSnap = await getDoc(rewardCircleStateRef(memberId, orgId));
...
export async function adminFindOrCreateRewardCircleMember(
  ...
  if (!memberId) {
    const memberDoc = await addDoc(membersCollection(), {
  ...
  await setDoc(
    memberOrgLinkRef(memberId, orgId),
    {
      orgId,
      memberId,
      joinedAt: now,
  ...
  await writeRewardCircleActivity({
    ...
    note: joinBonus > 0 ? "Member joined with welcome bonus." : "Member joined.",
  });
...
async function mergeRewardCircleMemberIdentity(input: {
  ...
  const snap = await getDoc(memberRef(input.memberId));
  ...
  await setDoc(memberRef(input.memberId), patch, { merge: true });
}
...
export async function getRewardCircleMemberState(
  ...
  const snap = await getDoc(rewardCircleStateRef(memberId, orgId));
...
async function mirrorRewardCircleMemberStateToOrg(
  ...
  await setDoc(
    rewardCircleMemberStateProjectionRef(state.orgId, state.memberId),
    state,
    { merge: true },
  );
}
...
async function updateRewardCircleMemberState(
  ...
  await Promise.all([
    setDoc(rewardCircleStateRef(state.memberId, state.orgId), state, {
      merge: true,
    }),
...
export async function writeRewardCircleActivity(
  ...
  const ref = await addDoc(activitiesCollection(), activityBase);
...
export async function awardRewardCirclePoints(
  ...
  const points =
    input.points > 0
      ? Math.round(safeNumber(input.points, 0))
      : Math.round(
          safeNumber(input.purchaseAmount, 0) *
            safeNumber(program.pointsPerDollar, 0),
        );
...
export async function redeemRewardCircleReward(
  ...
  const rewardSnap = await getDoc(
    rewardCircleRewardRef(input.orgId, input.rewardId),
  );
...
export async function getRewardCircleWalletByPhone(
  ...
  return {
    ...
    program,
    tierLabel:
      found.memberState.currentTierName || found.memberState.currentTier || "Starter",
  };
}
...
export async function getPublicRewardCircleProgramInfo(
  ...
  const rewardsQ = query(
    rewardCircleRewardsCollection(orgId),
    where("status", "==", "active"),
    where("isPublic", "==", true),
